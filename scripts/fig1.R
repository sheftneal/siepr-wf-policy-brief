...
#(top left) burned area (2020 numbers recorded as of 9/15 and scaled to estimate year-end totals)
    ...
    #National
      ...
      #historical data suggest about 85% of year-end burned area occurs by 9/15
        ...
        ba <- read_csv("data/fig1/burned_area_national.csv") %>% rename(ba_acres = burned_area) %>% 
    ...
    caba <- read_csv("data/fig1/burned_area_ca.csv") %>% dplyr::select(year, ba_acres) %>% dplyr::filter(!is.na(year)) 
...
#(bottom left) PM2.5 trends
    ...
    #Historically 9/15 PM2.5 averages 95% of year-end totals so 2020 values scaled accordingly
        ...
        epa2020 <- read_csv("data/fig1/pm25_regional_2020.csv")
...
#(top right) smoke days per year
        ...
        smoke_days <- read_csv("data/fig1/smoke_days_regional.csv")
        smoke_days_nat <- read_csv("data/fig1/smoke_days_national.csv")
...
#(bottom right)  Model predicted PM2.5 from wildfire smoke (through 2018) 
      ...
      #Data source: https://www.nber.org/papers/w27423?utm_campaign=ntwh&utm_medium=email&utm_source=ntwg25
        ...
        pmsmoke_regional <- read_csv("data/fig1/pm25_from_smoke_regional.csv")
        pmsmoke_nat <- read_csv("data/fig1/pm25_from_smoke_national.csv")
...
##########################################################################################################################################################################
    ...
    #National
        ...
        #extend to 2020 with dotted line to indicate estimates not final
          ...
          points(2020, ba$ba_ha[ba$year==2020], pch = 21, bg = 'white', col = 'gray',cex=1.75)
    ...
    #California
          ...
          points(2020, caba$ba_ha[caba$year==2020], pch = 21, bg = 'white', col = 'gray20',cex=1.75)
    ...
    #bottom left - PM2.5
          ...
          for (i in 2:dim(epa)[2]) {
            ...
            if (nm %in% c('West','Northwest')) {
              ...
              segments(x0 = 2019, x1 = 2020, y0 = epa[epa$year==2019,i], y1 =as.numeric(epa2020[nm]),col = col, lty=1)
              points(2020, as.numeric(epa2020[nm]) , pch =21, bg='white', col = col,cex=1.75)
          ...
          modtrend <- lm(National ~ year, data = epa)
          predtrend <- predict(modtrend)
          cf <- coef(modtrend)[2] %>% round(2) %>% as.numeric()
          
          
          lines(epa$year,predtrend,col="red3")
    ...
    # top right - smoke days 
          ...
          #loop over regions and plot time series for each through 2019
          for (i in 1:10) {
            f = smoke_days[smoke_days$epa_region == i & smoke_days$year<2020, ]
            #Highlight W and NW regions
            if (i %in% 9:10) { 
          ...
          #extend plot from 2019 to 2020 with dotted lines and points
          for (i in 1:10) {
            f = smoke_days[smoke_days$epa_region == i & smoke_days$year>=2019, ]
            #Highlight W and NW regions
            if (i %in% 9:10) {
              if (i == 10) {nm="Northwest"}
              if (i == 9) {nm = "West"}
              ...
              points(2020, predict(lm(smoke_days~year, data = f[f$year>2018,]), newdata = data.frame(year = 2020)) , pch =21, bg='white', col = 'gray20',cex=1.75)
              
            } else { #all other regions gray
              col="grey"
              segments(x0 = 2019, x1 = 2020, y0 = f$smoke_days[1], y1 = predict(lm(smoke_days~year, data = f[f$year>2018,]), newdata = data.frame(year = 2020)) ,col = col, lty=2)
              points(2020, predict(lm(smoke_days~year, data = f[f$year>2018,]), newdata = data.frame(year = 2020)) , pch =21, bg='white', col = 'gray',cex=1.75)
            }
          }
